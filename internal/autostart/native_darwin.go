...
func (NativeController) Status() (Status, error) {
	return statusFromNative(C.BrewtifyerAutostartStatus())
}
...
func (NativeController) SetEnabled(enabled bool) (Status, error) {
	...
	nativeStatus := C.BrewtifyerSetAutostartEnabled(nativeEnabled, &errorMessage)
	if errorMessage != nil {
...
func (NativeController) OpenSettings() error {
	if C.BrewtifyerOpenAutostartSettings() == 0 {
